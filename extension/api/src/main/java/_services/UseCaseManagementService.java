 ...
 */
interface UseCaseManagementService {
   ...
   * </ol>
   *
   * @param deploymentId the Id of the deployment to activate
   * @param reference the reference to the process application
   * @return a new {@link ProcessApplicationRegistration}
   * @throws AuthorizationException If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ProcessApplicationRegistration registerProcessApplication(String deploymentId, ProcessApplicationReference reference);

  /**
   * Deactivate a deployment for a given ProcessApplication. This removes the association between the process engine and the process
   * application and optionally removes the associated process definitions from the cache.
   *
   * @param deploymentId the Id of the deployment to deactivate
   * @param removeProcessDefinitionsFromCache indicates whether the process definitions should be removed from the deployment cache
   * @throws AuthorizationException If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
   ...
   * application and optionally removes the associated process definitions from the cache.
   *
   * @param deploymentIds the Ids of the deployments to deactivate
   * @param removeProcessDefinitionsFromCache indicates whether the process definitions should be removed from the deployment cache
   * @throws AuthorizationException If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ...
  /**
   * @return the name of the process application that is currently registered for the given deployment or 'null' if no process application
  ...
  /**
   * Gets the table name (including any configured prefix) for an entity like {@link Task}, {@link Execution} or the like.
   *
   * @throws AuthorizationException If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ...
  /**
   * Returns a new {@link JobDefinitionQuery} implementation, that can be used to dynamically query the job definitions.
   */
  JobDefinitionQuery createJobDefinitionQuery();

  /**
   * Forced synchronous execution of a job (eg. for administration or testing) The job will be executed, even if the process definition
   * and/or the process instance is in suspended state.
   *
   ...
   * <p>Note: for more complex activate commands use {@link #updateJobDefinitionSuspensionState()}.</p>
   *
   * @param activateJobs If true, all the {@link Job}s of the provided job definition will be activated too.
   * @throws ProcessEngineException If the job definition id is equal null.
   ...
   *
   * @param activateJobs If true, all the {@link Job}s of the provided job definition will be activated too.
   * @param activationDate The date on which the job definition will be activated. If null, the job definition is activated immediately.
   ...
   * <p>Note: for more complex suspend commands use {@link #updateJobDefinitionSuspensionState()}.</p>
   *
   * @param suspendJobs If true, all the {@link Job}s of the provided job definition will be suspended too.
   * @throws ProcessEngineException If the job definition id is equal null.
   * @throws AuthorizationException thrown if the current user does not possess
   * <ul>
   *   <li>{@link Permissions#UPDATE} on {@link Resources#PROCESS_DEFINITION}</li>
   ...
   *
   * @param suspendJobs If true, all the {@link Job}s of the provided job definition will be suspended too.
   * @param suspensionDate The date on which the job definition will be suspended. If null, the job definition is suspended immediately.
   * Note: The {@link JobExecutor} needs to be active to use this!
   * @throws ProcessEngineException If the job definition id is equal null.
   * @throws AuthorizationException thrown if the current user does not possess
   * <ul>
  ...
  /**
   * Activate or suspend jobs using a fluent builder. Specify the jobs by calling one of the <i>by</i> methods, like <i>byJobId</i>. To
   * update the suspension state call {@link UpdateJobSuspensionStateBuilder#activate()} or {@link
  ...
  /**
   * Activate or suspend job definitions using a fluent builder. Specify the job definitions by calling one of the <i>by</i> methods, like
   * <i>byJobDefinitionId</i>. To update the suspension state call
   * {@link UpdateJobDefinitionSuspensionStateBuilder#activate()} or {@link UpdateJobDefinitionSuspensionStateBuilder#suspend()}.
   *
   * @return the builder to update the suspension state
   */
   ...
   *
   * Whenever the JobExecutor fails to execute a job, this value is decremented. When it hits zero, the job is supposed to be dead and not
   * retried again. In that case, this method can be used to increase the number of retries.
   *
   ...
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION} and no {@link
   * ProcessInstancePermissions#RETRY_JOB} permission on {@link Resources#PROCESS_INSTANCE} and no {@link
   * ProcessDefinitionPermissions#RETRY_JOB} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * @throws AuthorizationException If the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} and no
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION} and no {@link Permissions#RETRY_JOB} permission
   * on {@link Resources#PROCESS_INSTANCE} and no {@link Permissions#RETRY_JOB} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * @param retries number of retries.
   * @throws BadUserRequestException if jobIds is null
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_SET_JOB_RETRIES}
   ...
   * retried again. In that case, this method can be used to increase the number of retries.
   *
   * Either jobIds or jobQuery has to be provided. If both are provided resulting list of affected jobs will contain jobs matching query as
   * well as jobs defined in the list.
   *
  ...
  /**
   * <p>
   * Set the number of retries of all <strong>failed</strong> {@link Job jobs} of the provided job definition id.
   * </p>
   *
   * <p>
   * Whenever the JobExecutor fails to execute a job, this value is decremented. When it hits zero, the job is supposed to be
   * <strong>failed</strong> and not retried again. In that case, this method can be used to increase the number of retries.
   * </p>
   *
   * <p>
   * {@link Incident Incidents} of the involved failed {@link Job jobs} will not be resolved using this method! When the execution of a job
   * was successful the corresponding incident will be resolved.
   * </p>
   *
  ...
  /**
   * Sets a new due date for the provided id. The offset between the old and the new due date can be cascaded to all follow-up jobs.
   * Cascading only works with timer jobs. When newDuedate is null, the job is executed with the next job executor run. In this case the
   * cascade parameter is ignored.
   *
   * @param jobId id of job to modify, cannot be null.
   * @param newDuedate new date for job execution
   ...
   * @param creationDateBased indicates whether the recalculation should be based on the creation date of the job or the current date
   * @throws AuthorizationException If the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * Jobs created after invoking this method receive the given priority. This setting overrides any setting specified in the BPMN 2.0
   * XML.</p>
   *
   ...
   * new jobs of the given definition receive the priority as specified in the BPMN 2.0 XML or the global default priority.</p>
   *
  ...
  /**
   * Returns the full stacktrace of the exception that occurs when the job with the given id was last executed. Returns null when the job
   * has no exception stacktrace.
   *
  ...
  /**
   * Get the deployments that are registered the engine's job executor. This set is only relevant, if the engine configuration property
   * <code>jobExecutorDeploymentAware</code> is set.
   *
   * @throws AuthorizationException If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ...
  /**
   * Register a deployment for the engine's job executor. This is required, if the engine configuration property
   * <code>jobExecutorDeploymentAware</code> is set. If set to false, the job executor will execute any job.
   *
   * @throws AuthorizationException If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ...
  /**
   * Unregister a deployment for the engine's job executor. If the engine configuration property <code>jobExecutorDeploymentAware</code> is
   * set, jobs for the given deployment will no longer get acquired.
   *
   * @throws AuthorizationException If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ...
  /**
   * Deletes all metrics events which are older than the specified timestamp and reported by the given reporter. If a parameter is null, all
   * metric events are matched in that regard.
   *
   * @param timestamp or null
   ...
   *
   * @throws ProcessEngineException if metrics reporting is disabled or the db metrics reporter is deactivated
   */
  void reportDbMetricsNow();

  /**
   * Creates a query to search for {@link Batch} instances.
   *
   * @since 7.5
   */
   ...
   * </p>
   *
   * @throws BadUserRequestException If no such batch can be found.
   * @throws AuthorizationException If the user has no {@link Permissions#UPDATE} permission on {@link Resources#BATCH}.
   * @since 7.5
   */
   ...
   *
   * If cascade is set to true the historic batch instances and the historic jobs logs are also removed.
   *

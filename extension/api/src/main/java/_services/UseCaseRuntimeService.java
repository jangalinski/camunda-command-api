 ...
 * @author Daniel Meyer
 */
interface UseCaseRuntimeService {
   ...
   * @throws AuthorizationException if the user has no {@link Permissions#CREATE} permission on {@link Resources#PROCESS_INSTANCE} and no
   * {@link Permissions#CREATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * practice.
   *
   * Note that a business key MUST be unique for the given process definition WHEN you have added a database constraint for it. In this
   * case, only Process instance from different process definition are allowed to have the same business key and the combination of
   * processdefinitionKey-businessKey must be unique.
   *
   * @param processDefinitionKey key of process definition, cannot be null.
   ...
   * @param processDefinitionKey key of process definition, cannot be null.
   * @param businessKey a key that uniquely identifies the process instance in the context of the given process definition.
   * @param caseInstanceId an id of a case instance to associate the process instance with a case instance.
   * @throws ProcessEngineException when no process definition is deployed with the given key.
   * @throws AuthorizationException if the user has no {@link Permissions#CREATE} permission on {@link Resources#PROCESS_INSTANCE} and no
   * {@link Permissions#CREATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * </p>
   *
   * @param messageName the 'name' of the message as specified as an attribute on the bpmn20 {@code <message name="messageName" />}
   * element.
   ...
   * {@link ProcessInstance}.</p>
   *
   * See {@link #startProcessInstanceByMessage(String)}. In addition, this method allows specifying a the payload of the message as a map of
   * process variables.
   *
   * @param messageName the 'name' of the message as specified as an attribute on the bpmn20 {@code <message name="messageName" />}
   * element.
   ...
   * {@link ProcessInstance}.</p>
   *
   * See {@link #startProcessInstanceByMessage(String)}. In addition, this method allows specifying the exactly version of the process
   ...
   * @param processDefinitionId the id of the process definition, cannot be null.
   * @return the {@link ProcessInstance} object representing the started process instance
   * @throws ProcessEngineException if no subscription to a message with the given name exists for the specified version of process
   * definition.
   * @throws AuthorizationException if the user has no {@link Permissions#CREATE} permission on {@link Resources#PROCESS_INSTANCE} and no
   * {@link Permissions#CREATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   * @since 7.3
   ...
   * {@link ProcessInstance}.</p>
   *
   * See {@link #startProcessInstanceByMessage(String, String)}. In addition, this method allows specifying the exactly version of the
   ...
   * {@link ProcessInstance}.</p>
   *
   * See {@link #startProcessInstanceByMessage(String, Map)}. In addition, this method allows specifying the exactly version of the process
   * definition with the given id.
   *
   * @param messageName the 'name' of the message as specified as an attribute on the bpmn20 {@code <message name="messageName" />} element,
   * cannot be null.
   ...
   * {@link ProcessInstance}.</p>
   *
   * See {@link #startProcessInstanceByMessage(String, String, Map)}. In addition, this method allows specifying the exactly version of the
   * process definition with the given id.
   *
   * @param messageName the 'name' of the message as specified as an attribute on the bpmn20 {@code <message name="messageName" />} element,
   * cannot be null.
   ...
   * @throws AuthorizationException if the user has no {@link Permissions#DELETE} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#DELETE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * @param processInstanceQuery query that will be used to fetch affected process instances. Cannot be null if processInstanceIds are
   * null.
   * @param deleteReason reason for deleting, which will be stored in the history. Can be null.
   * @throws BadUserRequestException when no process instance is found with the given id or id is null.
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_DELETE_RUNNING_PROCESS_INSTANCES}
   * permission on {@link Resources#BATCH}.
   */
   ...
   * Deletion propagates upward as far as necessary.
   *
   * @param processInstanceQuery query that will be used to fetch affected process instances. Cannot be null.
   * @param deleteReason reason for deleting, which will be stored in the history. Can be null.
   * @throws BadUserRequestException when no process instance is found with the given id or id is null.
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_DELETE_RUNNING_PROCESS_INSTANCES}
   * permission on {@link Resources#BATCH}.
   ...
   * Deletion propagates upward as far as necessary.
   *
   * If both process instances list and query are provided, process instances containing in both sets will be deleted.
   *
   ...
   * @param processInstanceId id of process instance to delete, cannot be null.
   * @param deleteReason reason for deleting, which will be stored in the history. Can be null.
   * @param skipCustomListeners if true, only the built-in {@link ExecutionListener}s are notified with the {@link
   ...
   * @param skipCustomListeners if true, only the built-in {@link ExecutionListener}s are notified with the {@link
   * ExecutionListener#EVENTNAME_END} event.
   * @param externallyTerminated indicator if deletion triggered from external context, for instance REST API call
   * @throws BadUserRequestException when no process instance is found with the given id or id is null.
   * @throws AuthorizationException if the user has no {@link Permissions#DELETE} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#DELETE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * {@link Permissions#DELETE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
  void deleteProcessInstances(List<String> processInstanceIds, String deleteReason, boolean skipCustomListeners,
   ...
   * {@link Permissions#DELETE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
  void deleteProcessInstancesIfExists(List<String> processInstanceIds, String deleteReason, boolean skipCustomListeners,
    boolean externallyTerminated,
    boolean skipSubprocesses);
   ...
   * {@link Permissions#DELETE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
  void deleteProcessInstanceIfExists(String processInstanceId, String deleteReason, boolean skipCustomListeners,
    boolean externallyTerminated, boolean skipIoMappings,
    boolean skipSubprocesses);

  /**
   * Finds the activity ids for all executions that are waiting in activities. This is a list because a single activity can be active
   * multiple times.
   *
   * Deletion propagates upward as far as necessary.
   *
   ...
   * @throws AuthorizationException if the user has no {@link Permissions#READ} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#READ_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   * The activity instance tree is aligned with the concept of scope in the BPMN specification. Activities that are "on the same level of
   * subprocess" (ie. part of the same scope, contained in the same subprocess) will have their activity instances at the same level in the
   * tree.</p>
   *
   * <h2>Examples:</h2>
   * <p><ul>
   ...
   * tree, all instances of both user tasks will be listed below the root activity instance. Reason: all activity instances are at the same
   * level of subprocess.</li>
   * <li>Usertask inside embedded subprocess: the activity instance three will have 3 levels: the root instance
   * representing the process instance itself, below it an activity instance representing the instance of the embedded subprocess, and below
   * this one, the activity instance representing the usertask.</li>
   * </ul></p>
   *
   ...
   * the same activity instance ids will be returned for the same activity instances. (However, there might be different executions
   * assigned, see below)</p>
   *
   ...
   * instance concept because the execution tree is in general not aligned with the activity / scope concept in BPMN. In general, there is a
   * n-1 relationship between Executions and ActivityInstances, ie. at a given point in time, an activity instance can be linked to multiple
   * executions. In addition, it is not guaranteed that the same execution that started a given activity instance will also end it. The
   * process engine performs several internal optimizations concerning the compacting of the execution tree which might lead to executions
   * being reordered and pruned. This can lead to situations where a given execution starts an activity instance but another execution ends
   * it. Another special case is the process instance: if the process instance is executing a non-scope activity (for example a user task)
   * below the process definition scope, it will be referenced by both the root activity instance and the user task activity instance.
   *
   ...
   *
   * Note that you need to provide the exact execution that is waiting for the signal if the process instance contains multiple executions.
   *
   ...
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} or no
   ...
   * @throws ProcessEngineException when no execution is found for the given executionId.
   * @throws AuthorizationException <li>if the user has no {@link Permissions#READ} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#READ_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}</li>
  ...
  /**
   * All variable values that are defined in the execution scope, without taking outer scopes into account. If you have many task local
   * variables and you only need a few, consider using {@link #getVariablesLocal(String, Collection)} for better performance.
   *
  ...
  /**
   * The variable values for all given variableNames, takes all variables into account which are visible from the given execution scope
  ...
  /**
   * The variable value.  Searching for the variable is done in all scopes that are visible to the given execution (including parent
   * scopes). Returns null when no variable value is found with the given name or when the value is set to null.
   *
   * @param executionId id of process instance or execution, cannot be null.
  ...
  /**
   * Returns a {@link TypedValue} for the variable. Searching for the variable is done in all scopes that are visible to the given execution
  ...
  /**
   * The variable value for an execution. Returns the value when the variable is set for the execution (and not searching parent scopes).
  ...
  /**
   * Returns a {@link TypedValue} for the variable. Returns the value when the variable is set for the execution (and not searching parent
   * scopes). Returns null when no variable value is found with the given name.
   *
   * @param executionId id of process instance or execution, cannot be null.
   * @param variableName name of variable, cannot be null.
   * @return the variable value or null if the variable is undefined.
  ...
  /**
   * Returns a {@link TypedValue} for the variable. Searching for the variable is done in all scopes that are visible to the given execution
   * (and not searching parent scopes). Returns null when no variable value is found with the given name.
   *
   * @param executionId id of process instance or execution, cannot be null.
   * @param variableName name of variable, cannot be null.
   * @param deserializeValue if false, a {@link SerializableValue} will not be deserialized
  ...
  /**
   * Update or create a variable for an execution.  If the variable does not already exist somewhere in the execution hierarchy (i.e. the
   * specified execution or any ancestor), it will be created in the process instance (which is the root execution).
   *
   ...
   * @throws ProcessEngineException when no execution is found for the given executionId.
   * @throws AuthorizationException if the user has none of the following:
  ...
  /**
   * Update or create a variable for an execution (not considering parent scopes). If the variable does not already exist, it will be
   * created in the given execution.
   *
  ...
  /**
   * Update or create given variables for an execution (including parent scopes). If the variables are not already existing, they will be
   * created in the process instance (which is the root execution).
   *
   * @param executionId id of the process instance or the execution, cannot be null.
  ...
  /**
   * Update or create given variables for an execution (not considering parent scopes). If the variables are not already existing, it will
   * be created in the given execution.
   *
  ...
  /**
   * Creates a new {@link IncidentQuery} instance, that can be used to query incidents.
   */
  IncidentQuery createIncidentQuery();

  /**
   * Creates a new {@link EventSubscriptionQuery} instance, that can be used to query event subscriptions.
   */
  EventSubscriptionQuery createEventSubscriptionQuery();

  /**
   * Creates a new {@link VariableInstanceQuery} instance, that can be used to query variable instances.
   */
   ...
   * execution is stopped, so the <i>token state</i> will not change. However, actions that do not change token state, like setting/removing
   ...
   * This means that the execution is stopped, so the <i>token state</i> will not change. However, actions that do not change token state,
   * like setting/removing variables, etc. will succeed.</p>
   *
   ...
   * one process instance from the hierarchy will not activate other process instances from that hierarchy.</p>
   *
  ...
  /**
   * Activate or suspend process instances using a fluent builder. Specify the instances by calling one of the <i>by</i> methods, like
   * <i>byProcessInstanceId</i>. To update the suspension state call
   * {@link UpdateProcessInstanceSuspensionStateBuilder#activate()} or {@link UpdateProcessInstanceSuspensionStateBuilder#suspend()}.
   *
  ...
  /**
   * Notifies the process engine that a signal event of name 'signalName' has been received. Delivers the signal to all executions waiting
   * on the signal and to all process definitions that can started by this signal. <p/>
   *
   * <strong>NOTE:</strong> Notification and instantiation happen synchronously.
   *
   * @param signalName the name of the signal event
   * @throws AuthorizationException <li>if notify an execution and the user has no {@link Permissions#UPDATE} permission on {@link
   * Resources#PROCESS_INSTANCE} or no {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.</li>
  ...
  /**
   * Notifies the process engine that a signal event of name 'signalName' has been received. This method delivers the signal to a single
   * execution, being the execution referenced by 'executionId'. The waiting execution is notified synchronously.
   *
   * Note that you need to provide the exact execution that is waiting for the signal if the process instance contains multiple executions.
   *
   * @param signalName the name of the signal event
   * @param executionId id of the process instance or the execution to deliver the signal to
   * @throws ProcessEngineException if no such execution exists or if the execution has not subscribed to the signal
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   *
   * @param signalName the name of the signal event
   * @param executionId the id of the process instance or the execution to deliver the signal to
   * @param processVariables a map of variables added to the execution(s)
   * @throws ProcessEngineException if no such execution exists or if the execution has not subscribed to the signal
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
  ...
  /**
   * Notifies the process engine that a message event with name 'messageName' has been received and has been correlated to an execution with
   * id 'executionId'.
   *
   * The waiting execution is notified synchronously.
   *
   * Note that you need to provide the exact execution that is waiting for the message if the process instance contains multiple
   * executions.
   *
   * @param messageName the name of the message event
   * @param executionId the id of the process instance or the execution to deliver the message to
   * @throws ProcessEngineException if no such execution exists or if the execution has not subscribed to the signal
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */
  ...
  /**
   * Notifies the process engine that a message event with the name 'messageName' has been received and has been correlated to an execution
   * with id 'executionId'.
   *
   * The waiting execution is notified synchronously.
   *
   ...
   *
   * @param messageName the name of the message. Corresponds to the 'name' element of the message defined in BPMN 2.0 Xml. Can be null to
   * correlate by other criteria (businessKey, processInstanceId, correlationKeys) only.
  ...
  /**
   * Correlates a message to either an execution that is waiting for this message or a process definition that can be started by this
   * message.
   *
   * Notification and instantiation happen synchronously.
   *
   * @param messageName the name of the message event; if null, matches any event
   ...
   *
   * @param messageName the name of the message event; if null, matches any event
   * @param businessKey the business key of process instances to correlate against
   * @throws MismatchingMessageCorrelationException if none or more than one execution or process definition is correlated
   ...
   *
   * @param messageName the name of the message event; if null, matches any event
   * @param correlationKeys a map of key value pairs that are used to correlate the message to an execution
   * @throws MismatchingMessageCorrelationException if none or more than one execution or process definition is correlated
   ...
   * @param messageName the name of the message event; if null, matches any event
   * @param businessKey the business key of process instances to correlate against
   * @param processVariables a map of variables added to the execution or newly created process instance
   * @throws MismatchingMessageCorrelationException if none or more than one execution or process definition is correlated
   * @throws ProcessEngineException if messageName is null and businessKey is null
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} or no
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
  ...
  /**
   * Define a modification of a process instance in terms of activity cancellations and instantiations via a fluent builder. Instructions
   ...
   * The builder can be used to set further properties and specify instantiation instructions to start the instance at any set of activities
   * in the process. If no instantiation instructions are set then the instance start at the default start activity.
   *
   * @param processDefinitionId the id of the process definition, cannot be <code>null</code>.
  ...
  /**
   * Returns a fluent builder to start a new process instance in the latest version of the process definition with the given key. The
   * builder can be used to set further properties and specify instantiation instructions to start the instance at any set of activities in
   * the process. If no instantiation instructions are set then the instance start at the default start activity.
   *
   * @param processDefinitionKey the key of the process definition, cannot be <code>null</code>.
   * @return a builder to create a process instance of the definition
   */
  ProcessInstantiationBuilder createProcessInstanceByKey(String processDefinitionKey);

  /**
   * Creates a migration plan to migrate process instance between different process definitions. Returns a fluent builder that can be used
   * to specify migration instructions and build the plan.
   *
  ...
  /**
   * Executes a migration plan for a given list of process instances. The migration can either be executed synchronously or asynchronously.
   * A synchronously migration blocks the caller until the migration was completed. The migration can only be successfully completed if all
   * process instances can be migrated.
   *
   * If the migration is executed asynchronously a {@link Batch} is immediately returned. The migration is then executed as jobs from the
   * process engine and the batch can be used to track the progress of the migration. The Batch splits the migration in smaller chunks which
   ...
   * Returns a fluent builder that can be used to specify modification instructions and set process instances that should be modified.
   *
   * The modification can either be executed synchronously or asynchronously. A synchronously modification blocks the caller until the
   * modification was completed. The modification can only be successfully completed if all process instances can be modified.
   *
   * If the modification is executed asynchronously a {@link Batch} is immediately returned. The modification is then executed as jobs from
   * the process engine and the batch can be used to track the progress of the modification. The Batch splits the modification in smaller
   * chunks which will be executed independently.
   *
   ...
   *
   * @param processDefinitionId the id of the process definition, cannot be null.
   * @throws ProcessEngineException when no process definition is deployed with the given key or a process instance is still active.
   * @throws AuthorizationException if the user has not all of the following permissions
   * <ul>
   ...
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permission on {@link Resources#PROCESS_INSTANCE} and no
   * {@link Permissions#UPDATE_INSTANCE} permission on {@link Resources#PROCESS_DEFINITION}.
   */

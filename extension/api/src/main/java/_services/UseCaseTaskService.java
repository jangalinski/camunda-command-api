...
interface UseCaseTaskService {
   ...
   *
   * @throws AuthorizationException if the user has no {@link Permissions#CREATE} permission on {@link Resources#TASK}.
   */
  Task newTask();
  ...
  /**
   * Saves the given task to the persistent data store. If the task is already present in the persistent store, it is updated. After a new
   * task has been saved, the task instance passed into this method is updated with the id of the newly created task.
   *
   * @param task the task, cannot be null.
   * @throws AuthorizationException If the task is already present and the user has no {@link Permissions#UPDATE} permission on {@link
   * Resources#TASK} or no {@link Permissions#UPDATE_TASK} permission on {@link Resources#PROCESS_DEFINITION}. Or if the task is not present
   * and the user has no {@link Permissions#CREATE} permission on {@link Resources#TASK}.
   */
   ...
   *
   * @param taskId The id of the task that will be deleted, cannot be null. If no task exists with the given taskId, the operation is
   * ignored.
   * @throws ProcessEngineException when an error occurs while deleting the task or in case the task is part of a running process or case
   * instance.
   ...
   *
   * @param taskIds The id's of the tasks that will be deleted, cannot be null. All id's in the list that don't have an existing task will
   * be ignored.
   * @throws ProcessEngineException when an error occurs while deleting the tasks or in case one of the tasks is part of a running process
   * or case instance.
   * @throws AuthorizationException If the user has no {@link Permissions#DELETE} permission on {@link Resources#TASK}.
   */
  ...
  /**
   * Claim responsibility for a task: the given user is made {@link Task#getAssignee() assignee} for the task. The difference with {@link
   * #setAssignee(String, String)} is that here a check is done if the task already has a user assigned to it. No check is done whether the
   * user is known by the identity component.
   *
   ...
   * @throws AuthorizationException If the user has no {@link Permissions#UPDATE} permission on {@link Resources#TASK} or no {@link
   * Permissions#UPDATE_TASK} permission on {@link Resources#PROCESS_DEFINITION} (if the task is part of a running process instance).
   */
   ...
   *
   * This method is typically called by a task list user interface after a task form has been submitted by the {@link Task#getAssignee()
   * assignee}.
   *
   ...
   *
   * This means that the {@link Task#getAssignee() assignee} is set and the {@link Task#getDelegationState() delegation state} is set to
   * {@link DelegationState#PENDING}. If no owner is set on the task, the owner is set to the current {@link Task#getAssignee() assignee} of
   * the task. The new assignee must use {@link UseCaseTaskService#resolveTask(String)} to report back to the owner. Only the owner can
   * {@link UseCaseTaskService#complete(String) complete} the task.
   *
  ...
  /**
   * Marks that the {@link Task#getAssignee() assignee} is done with the task {@link UseCaseTaskService#delegateTask(String, String)
   * delegated} to her and that it can be sent back to the {@link Task#getOwner() owner}. Can only be called when this task is {@link
   * DelegationState#PENDING} delegation. After this method returns, the {@link Task#getDelegationState() delegation state} is set to {@link
   * DelegationState#RESOLVED} and the task can be {@link UseCaseTaskService#complete(String) completed}.
   *
  ...
  /**
   * Marks that the {@link Task#getAssignee() assignee} is done with the task {@link UseCaseTaskService#delegateTask(String, String)
   * delegated} to her and that it can be sent back to the {@link Task#getOwner() owner} with the provided variables. Can only be called
   * when this task is {@link DelegationState#PENDING} delegation. After this method returns, the {@link Task#getDelegationState()
   * delegation state} is set to {@link DelegationState#RESOLVED} and the task can be {@link UseCaseTaskService#complete(String)
   * completed}.
   *
   * @throws ProcessEngineException when no task exists with the given id.
   * @throws AuthorizationException If the user has no {@link Permissions#UPDATE} permission on {@link Resources#TASK} or no {@link
   ...
   *
   * This method is typically called by a task list user interface after a task form has been submitted by the {@link Task#getAssignee()
   * assignee} and the required task parameters have been provided.
   *
   * @param taskId the id of the task to complete, cannot be null.
   ...
   * @param taskId the id of the task to complete, cannot be null.
   * @param variables task parameters. May be null or empty.
   * @param deserializeValues if false, returned {@link SerializableValue}s will not be deserialized (unless they are passed into this
   * method as a deserialized value or if the BPMN process triggers deserialization)
  ...
  /**
   * Retrieves the {@link IdentityLink}s associated with the given task. Such an {@link IdentityLink} informs how a certain identity (eg.
   * group or user) is associated with a certain task (eg. as candidate, assignee, etc.)
   *
   * @throws ProcessEngineException when the task doesn't exist.
   * @throws AuthorizationException If the user has no {@link Permissions#READ} permission on {@link Resources#TASK} or no {@link
   * Permissions#READ_TASK} permission on {@link Resources#PROCESS_DEFINITION} (if the task is part of a running process instance).
   */
  ...
  /**
   * Set variable on a task. If the variable is not already existing, it will be created in the most outer scope.  This means the process
   ...
   *
   * @throws ProcessEngineException when the task doesn't exist.
   * @throws AuthorizationException <p>In case of standalone tasks:
   ...
   * the user has no {@link TaskPermissions#READ_VARIABLE} permission on {@link Resources#TASK} and no {@link
   * ProcessDefinitionPermissions#READ_TASK_VARIABLE} permission on {@link Resources#PROCESS_DEFINITION}</li></p>
   */
  ...
  /**
   * Get all variables and search in the task scope and if available also the execution scopes. If you have many variables and you only need
   * a few, consider using {@link #getVariables(String, Collection)} for better performance.
   *
   * @throws ProcessEngineException when the task doesn't exist.
   * @throws AuthorizationException <p>In case of standalone tasks:
   * <li>if the user has no {@link Permissions#READ} permission on {@link Resources#TASK} or</li>
  ...
  /**
   * Get all variables and search only in the task scope. If you have many task local variables and you only need a few, consider using
  ...
  /**
   * Removes all variables in the given collection from the task (not considering parent scopes). Non existing variable names are simply
   * ignored.
   *
   * @throws ProcessEngineException when the task doesn't exist.
   * @throws AuthorizationException If the user has none of the following:
   ...
   * user. This behavior can be toggled in the process engine configuration using the property <code>legacyUserOperationLog</code> (default
   * false). To restore the engine's previous behavior, set the flag to <code>true</code>.</p>
   *
   * @see HistoryService#createUserOperationLogQuery()
   * @deprecated This method has been deprecated as of camunda BPM 7.1. It has been replaced with the operation log. See {@link
   * UserOperationLogEntry} and {@link UserOperationLogQuery}.
   */
  @Deprecated
  ...
  /**
   * Add a new attachment to a task and/or a process instance and use an input stream to provide the content please use method in runtime
   * service to operate on process instance.
   *
  ...
  /**
   * Add a new attachment to a task and/or a process instance and use an url as the content please use method in runtime service to operate
   * on process instance
   *
   * Either taskId or processInstanceId has to be provided
   *

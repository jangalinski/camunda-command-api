...
/**
 * Service which provides access to {@link CaseInstance case instances} and {@link CaseExecution case executions}.
 *
 * @author Roman Smirnov
 * @since 7.2
 */
interface UseCaseCaseService {
   ...
   * certain identifier that has a clear business meaning. This business key can then be used to easily look up that case instance, see
   * {@link CaseInstanceQuery#caseInstanceBusinessKey(String)}. Providing such a business key is definitely a best practice.</p>
   *
   * <p>Note that a business key MUST be unique for the given case definition WHEN
   * you have added a database constraint for it. In this case, only case instance from different case definition are allowed to have the
   * same business key and the combination of caseDefinitionKey-businessKey must be unique.</p>
   *
   * @param caseDefinitionKey the key of the case definition to instantiate
   * @param businessKey a key that uniquely identifies the case instance in the context of the given case definition.
   * @throws NotValidException when the given case definition key is null.
   * @throws NotFoundException when no case definition is deployed with the given key.
   * @throws ProcessEngineException when an internal exception happens during the execution of the command
   */
   ...
   *     <li>{@link HumanTask}: a new {@link Task user task} is instantiated</li>
   ...
   * @throws NotAllowedException when the transition is not allowed to be done or when the case execution is a case instance
   * @throws ProcessEngineException when an internal exception happens during the execution of the command.
   */
   ...
   * Performs a transition from state <code>ENABLED</code> to state <code>DISABLED</code>.</p>
   *
   * <p>According to CMMN 1.0 specification, the state <code>DISABLED</code> means that the
   * {@link Stage} or {@link Task} related to the case execution should not be executed in this case instance.</p>
   *
   * <p>If the given case execution has a parent case execution, that parent
   * case execution will be notified that the given case execution has been disabled. This can lead to a completion of the parent case
   * execution if the completion criteria are fulfilled.</p>
   *
   ...
   * Performs a transition from state <code>DISABLED</code> to state <code>ENABLED</code>.</p>
   *
   * <p>According to CMMN 1.0 specification, the state <code>DISABLED</code> means that the
   * {@link Stage} or {@link Task} related to the case execution pends for a decision to become <code>ACTIVE</code> or
   ...
   * Performs a transition from state <code>ACTIVE</code> to state <code>COMPLETED</code>.</p>
   *
   ...
   * belongs to. Once closed, no further work or modifications are allowed for the case instance. Performs a transition from state
   * <code>COMPLETED</code> to state <code>CLOSED</code>.</p>
   *
   * @param caseExecutionId the id of the case execution to close the case instance for
   * @throws NotValidException when the given case execution id is null
   * @throws NotFoundException when no case execution is found for the given case execution id
   * @throws NotAllowedException when the transition is not allowed to be done
   * @throws ProcessEngineException when an internal exception happens during the execution of the command.
   ...
   * Performs the transition from <code>ACTIVE</code> to state <code>TERMINATED</code> if the case execution belongs to a case model or a
   * task or a stage. Performs the transition from <code>AVAILABLE</code> to state <code>TERMINATED</code> if the case execution belongs to
   * a milestone.</p>
   *
  ...
  /**
   * <p>Terminates the case execution identified by the given id.
   * Performs the transition from <code>ACTIVE</code> to state <code>TERMINATED</code> if the case execution belongs to either a case model
   * or a task or a stage. Performs the transition from <code>AVAILABLE</code> to state <code>TERMINATED</code> if the case execution
   * belongs to a milestone.</p>
   *
   ...
   * @return a {@link CaseExecutionCommandBuilder fluent builder} for defining a command for a case execution
   */
   ...
   * into account which are visible from the given case execution scope (including parent scopes).</p>
   *
   * @param caseExecutionId the id of a case instance or case execution, cannot be null
   ...
   * exist, they are created in the case instance (which is the root execution). Otherwise existing variables are updated.</p>
   *
   ...
   * exist, it is created in the case instance (which is the root execution). Otherwise, the existing variable is updated.</p>
   *

...
/**
 * Service that provides access to {@link ExternalTask} instances. External tasks represent work items that are processed externally and
 * independently of the process engine.
 *
 * @author Thorben Lindhauer
 * @author Christopher Zell
 */
interface UseCaseExternalTaskService {
   ...
   * @see {@link UseCaseExternalTaskService#fetchAndLock(int, String, boolean)}.
   */
   ...
   * The following parameters must be specified: A worker id, a maximum number of tasks to fetch and a flag that indicates whether priority
   * should be regarded or not. The builder allows to specify multiple topics to fetch tasks for and individual lock durations. For every
   * topic, variables can be fetched in addition.Is the priority enabled the tasks with the highest priority are fetched.</p>
   *
   * <p>Returned tasks are locked for the given worker until
   * <code>now + lockDuration</code> expires.
   * Locked tasks cannot be fetched or completed by other workers. When the lock time has expired, a task may be fetched and locked by other
   * workers.</p>
   *
   * <p>Returns at most <code>maxTasks</code> tasks. The tasks are arbitrarily
   * distributed among the specified topics. Example: Fetching 10 tasks of topics "a"/"b"/"c" may return 3/3/4 tasks, or 10/0/0 tasks,
   * etc.</p>
   *
   * <p>May return less than <code>maxTasks</code> tasks, if there exist not enough
   * unlocked tasks matching the provided topics or if parallel fetching by other workers results in locking failures.</p>
   *
   * <p>
   ...
   * to the process instance before continuing execution. The given task must be assigned to the worker.</p>
   *
   * @param externalTaskId the id of the external to complete
   * @param workerId the id of the worker that completes the task
   * @param variables a map of variables to set on the execution (non-local) the external task is assigned to
   * @throws NotFoundException if no external task with the given id exists
   * @throws BadUserRequestException if the task is assigned to a different worker
   * @throws AuthorizationException thrown if the current user does not possess any of the following permissions:
   * <ul>
   ...
   * The task must be assigned to the given worker. The number of retries left can be specified. In addition, a timeout can be provided,
   * such that the task cannot be fetched before <code>now + retryTimeout</code> again.</p>
   *
   ...
   * @param errorMessage short error message related to this failure. This message can be retrieved via {@link
   * ExternalTask#getErrorMessage()} and is used as the incident message in case <code>retries</code> is <code>null</code>. May be
   * <code>null</code>.
   * @param retries the number of retries left. External tasks with 0 retries cannot be fetched anymore unless the number of retries is
   * increased via API. Must be >= 0.
   ...
   * ExternalTask#getErrorMessage()} and is used as the incident message in case <code>retries</code> is <code>null</code>. May be
   * <code>null</code>.
   * @param errorDetails full error message related to this failure. This message can be retrieved via {@link
   * UseCaseExternalTaskService#getExternalTaskErrorDetails(String)} ()}
   * @param retries the number of retries left. External tasks with 0 retries cannot be fetched anymore unless the number of retries is
   * increased via API. Must be >= 0.
   * @param retryTimeout the timeout before the task can be fetched again. Must be >= 0.
   * @throws NotFoundException if no external task with the given id exists
   ...
   * </ul>
   */
  public void handleFailure(String externalTaskId, String workerId, String errorMessage, String errorDetails, int retries,
    long retryTimeout);

  /**
   * <p>Signals that an business error appears, which should be handled by the process engine.
   * The task must be assigned to the given worker. The error will be propagated to the next error handler. Is no existing error handler for
   * the given bpmn error the activity instance of the external task ends.</p>
   *
  ...
  /**
   * Sets the retries for an external task. If the new value is 0, a new incident with a <code>null</code> message is created. If the old
  ...
  /**
   * Sets the retries for external tasks. If the new value is 0, a new incident with a <code>null</code> message is created. If the old
   * value is 0 and the new value is greater than 0, an existing incident is resolved.
   *
  ...
  /**
   * Sets the retries for external tasks asynchronously as batch. The returned batch can be used to track the progress. If the new value is
   * 0, a new incident with a <code>null</code> message is created. If the old value is 0 and the new value is greater than 0, an existing
   ...
   * @throws NotFoundException if no external task with one of the given id exists
   * @throws BadUserRequestException if the ids are null or the number of retries is negative
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_SET_EXTERNAL_TASK_RETRIES}
   ...
   * {@link UpdateExternalTaskRetriesBuilder#set(int)} or {@link UpdateExternalTaskRetriesBuilder#setAsync(int)}.
   *
   * @since 7.8
   */
   ...
   * </p>
   *
   * @return a new {@link ExternalTaskQuery} that can be used to dynamically query for external tasks.
   */
  public ExternalTaskQuery createExternalTaskQuery();

  /**
   * Returns the full error details that occurred while running external task with the given id. Returns null when the external task has no
   * error details.
   *
   * @param externalTaskId id of the external task, cannot be null.
   * @throws ProcessEngineException When no external task exists with the given id.

 ...
 * @author Daniel Meyer
 */
interface UseCaseIdentityService {
   ...
   *
   * @return true if this identity service implementation provides read-only access to the user repository, false otherwise.
   */
   ...
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permissions on {@link Resources#USER} (update existing
   * user) or if user has no {@link Permissions#CREATE} permissions on {@link Resources#USER} (save new user).
   */
   ...
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permissions on {@link Resources#GROUP} (update existing
   * group) or if user has no {@link Permissions#CREATE} permissions on {@link Resources#GROUP} (save new group).
   */
  ...
  /**
   * Delete the membership of the user in the group. When the group or user don't exist or when the user is not a member of the group, this
   ...
   *
   * @param tenantId id for the new tenant, cannot be <code>null</code>.
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#CREATE} permissions on {@link Resources#TENANT}.
   */
   ...
   *
   * @param tenant the tenant to save. Cannot be <code>null</code>.
   * @throws RuntimeException when a tenant with the same name already exists.
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#UPDATE} permissions on {@link Resources#TENANT} (update existing
   * tenant) or if user has no {@link Permissions#CREATE} permissions on {@link Resources#TENANT} (save new tenant).
   */
   ...
   *
   * @param tenantId id of the tenant that should be deleted, cannot be
   * <code>null</code>.
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#DELETE} permissions on {@link Resources#TENANT}.
   */
   ...
   *
   * @param tenantId the id of the tenant, cannot be null.
   * @param userId the id of the user, cannot be null.
   * @throws RuntimeException when the given tenant or user doesn't exist or the user is already a member of this tenant.
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#CREATE} permissions on {@link Resources#TENANT_MEMBERSHIP}.
   */
   ...
   * @throws RuntimeException when the given tenant or group doesn't exist or when the group is already a member of this tenant.
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#CREATE} permissions on {@link Resources#TENANT_MEMBERSHIP}.
   */
  ...
  /**
   * Deletes the membership between the given user and tenant. The operation is ignored when the given user, tenant or membership don't
   * exist.
   *
   * @param tenantId the id of the tenant, cannot be null.
   * @param userId the id of the user, cannot be null.
   * @throws UnsupportedOperationException if identity service implementation is read only. See {@link #isReadOnly()}
   * @throws AuthorizationException if the user has no {@link Permissions#DELETE} permissions on {@link Resources#TENANT_MEMBERSHIP}.
   */
  void deleteTenantUserMembership(String tenantId, String userId);

  /**
   * Deletes the membership between the given group and tenant. The operation is ignored when the given group, tenant or membership don't
   * exist.
   *
   * @param tenantId the id of the tenant, cannot be null.
   * @param groupId the id of the group, cannot be null.
  ...
  /**
   * Check a given password against the configured {@link PasswordPolicy}. The result is returned as {@link PasswordPolicyResult} which
   * contains all passed and violated rules as well as a flag indicating if the password is valid.
   *
   * @param password the password that should be tested
  ...
  /**
   * Check a given password against a given {@link PasswordPolicy}. The result is returned as {@link PasswordPolicyResult} which contains
   * all passed and violated rules as well as a flag indicating if the password is valid.
   *
   * @param policy the {@link PasswordPolicy} against which the password is tested
   * @param password the password that should be tested
   * @return a {@link PasswordPolicyResult} containing passed and failed rules
   */
   ...
   *
   * @return the current {@link PasswordPolicy} or <code>null</code> if no policy is set or the configured policy is disabled.
   */
  PasswordPolicy getPasswordPolicy();

  /**
   * Passes the authenticated user id for this thread. All service method (from any service) invocations done by the same thread will have
   * access to this authenticatedUserId. Should be followed by a call to {@link #clearAuthentication()} once the interaction is terminated.
   *
  ...
  /**
   * Passes the authenticated user id and groupIds for this thread. All service method (from any service) invocations done by the same
   * thread will have access to this authentication. Should be followed by a call to {@link #clearAuthentication()} once the interaction is
   * terminated.
   *
   * @param authenticatedUserId the id of the current user.
  ...
  /**
   * Passes the authenticated user id, group ids and tenant ids for this thread. All service method (from any service) invocations done by
   * the same thread will have access to this authentication. Should be followed by a call to {@link #clearAuthentication()} once the
   ...
   */
  @Deprecated
  void setUserAccount(String userId, String userPassword, String accountName, String accountUsername, String accountPassword,

...
/**
 * Service exposing information about ongoing and past process instances.  This is different from the runtime information in the sense that
 * this runtime information only contains the actual runtime state at any given moment and it is optimized for runtime process execution
 * performance.  The history information is optimized for easy querying and remains permanent in the persistent storage.
 *
 * @author Christian Stettler
 * @author Tom Baeyens
 * @author Joram Barrez
 */
interface UseCaseHistoryService {
   ...
   *
   * If the user has no {@link Permissions#READ_HISTORY} permission on {@link Resources#DECISION_DEFINITION} then the result of the query is
   * empty.
   */
  HistoricDecisionInstanceQuery createHistoricDecisionInstanceQuery();

  /**
   * Deletes historic task instance.  This might be useful for tasks that are {@link TaskService#newTask() dynamically created} and then
   * {@link TaskService#complete(String) completed}. If the historic task instance doesn't exist, no exception is thrown and the method
   * returns normal.
   *
   * @throws AuthorizationException If the user has no {@link Permissions#DELETE_HISTORY} permission on {@link
   * Resources#PROCESS_DEFINITION}.
   */
  void deleteHistoricTaskInstance(String taskId);

  /**
   * Deletes historic process instance. All historic activities, historic task and historic details (variable updates, form properties) are
   * deleted as well.
   *
   * @throws AuthorizationException If the user has no {@link Permissions#DELETE_HISTORY} permission on {@link
   * Resources#PROCESS_DEFINITION}.
  ...
  /**
   * Deletes historic process instance. All historic activities, historic task and historic details (variable updates, form properties) are
   * deleted as well. Does not fail if a process instance was not found.
   *
   * @throws AuthorizationException If the user has no {@link Permissions#DELETE_HISTORY} permission on {@link
   * Resources#PROCESS_DEFINITION}.
   */
  ...
  /**
   * Deletes historic process instances. All historic activities, historic task and historic details (variable updates, form properties) are
   * deleted as well.
   *
  ...
  /**
   * Deletes historic process instances and all related historic data in bulk manner. DELETE SQL statement will be created for each entity
   * type. They will have list of given process instance ids in IN clause. Therefore, DB limitation for number of values in IN clause must
   * be taken into account.
   *
   * @param processInstanceIds list of process instance ids for removal
   * @throws BadUserRequestException when no process instances are found with the given ids or ids are null or when some of the process
   * instances are not finished yet
   * @throws AuthorizationException If the user has no {@link Permissions#DELETE_HISTORY} permission on {@link
   * Resources#PROCESS_DEFINITION}.
   */
  ...
  /**
   * Schedules history cleanup job at batch window start time. The job will delete historic data for finished process, decision and case
   * instances, and batch operations taking into account {@link ProcessDefinition#getHistoryTimeToLive()}, {@link
   * DecisionDefinition#getHistoryTimeToLive()}, {@link CaseDefinition#getHistoryTimeToLive()}, {@link
   * ProcessEngineConfigurationImpl#getBatchOperationHistoryTimeToLive()} and {@link ProcessEngineConfigurationImpl#getBatchOperationsForHistoryCleanup()}
   * values.
   *
   * @return history cleanup job. NB! As of v. 7.9.0, method does not guarantee to return a job. Use {@link #findHistoryCleanupJobs()}
   * instead.
   * @throws AuthorizationException If the user has no {@link Permissions#DELETE_HISTORY} permission on {@link
   * Resources#PROCESS_DEFINITION}
   */
   ...
   * values.
   *
   * @param immediatelyDue must be true if cleanup must be scheduled at once, otherwise is will be scheduled according to configured batch
   * window
   ...
   *
   * @return job entity
   * @deprecated As of v. 7.9.0, because there can be more than one history cleanup job at once, use {@link #findHistoryCleanupJobs}
   * instead.
   */
  @Deprecated
  ...
  /**
   * Deletes historic process instances asynchronously. All historic activities, historic task and historic details (variable updates, form
   * properties) are deleted as well.
   *
   * @throws BadUserRequestException when no process instances is found with the given ids or ids are null.
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_DELETE_FINISHED_PROCESS_INSTANCES}
  ...
  /**
   * Deletes historic process instances asynchronously based on query. All historic activities, historic task and historic details (variable
   * updates, form properties) are deleted as well.
   *
   * @throws BadUserRequestException when no process instances is found with the given ids or ids are null.
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_DELETE_FINISHED_PROCESS_INSTANCES}
   * permission on {@link Resources#BATCH}.
  ...
  /**
   * Deletes historic process instances asynchronously based on query and a list of process instances. Query result and list of ids will be
   * merged. All historic activities, historic task and historic details (variable updates, form properties) are deleted as well.
   *
   * @throws BadUserRequestException when no process instances is found with the given ids or ids are null.
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_DELETE_FINISHED_PROCESS_INSTANCES}
   * permission on {@link Resources#BATCH}.
   ...
   *
   * @throws AuthorizationException For entries related to process definition keys: If the user has neither {@link
   * Permissions#DELETE_HISTORY} permission on {@link Resources#PROCESS_DEFINITION} nor {@link UserOperationLogCategoryPermissions#DELETE}
   * permission on {@link Resources#OPERATION_LOG_CATEGORY}. For entries not related to process definition keys: If the user has no {@link
   * UserOperationLogCategoryPermissions#DELETE} permission on {@link Resources#OPERATION_LOG_CATEGORY}.
   */
  ...
  /**
   * Deletes historic case instances and all related historic data in bulk manner. DELETE SQL statement will be created for each entity
   * type. They will have list of given case instance ids in IN clause. Therefore, DB limitation for number of values in IN clause must be
   * taken into account.
   *
   ...
   * @deprecated Note that this method name is not expressive enough, because it is also possible to delete the historic decision instance
   * by the instance id. Therefore use {@link #deleteHistoricDecisionInstanceByDefinitionId} instead to delete the historic decision
   * instance by the definition id.
   */
  @Deprecated
  void deleteHistoricDecisionInstance(String decisionDefinitionId);

  /**
   * Deletes decision instances and all related historic data in bulk manner. DELETE SQL statement will be created for each entity type.
   * They will have list of given decision instance ids in IN clause. Therefore, DB limitation for number of values in IN clause must be
   * taken into account.
   *
   ...
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_DELETE_DECISION_INSTANCES}
   * permission on {@link Resources#BATCH}.
   */
  ...
  /**
   * Deletes historic decision instances asynchronously based on query and a list of decision instances, whereby query result and list of
   * ids will be merged.
   *
   * @throws BadUserRequestException when no decision instances are found with the given ids or ids are null.
   * @throws AuthorizationException If the user has no {@link Permissions#CREATE} or {@link BatchPermissions#CREATE_BATCH_DELETE_DECISION_INSTANCES}
  ...
  /**
   * Deletes a historic variable instance by its id. All related historic details (variable updates, form properties) are deleted as well.
   *
   * @param variableInstanceId the id of the variable instance
   * @throws BadUserRequestException when the historic variable instance is not found by the given id or if id is null
   * @throws AuthorizationException If the variable instance has a process definition key and the user has no {@link
   * Permissions#DELETE_HISTORY} permission on {@link Resources#PROCESS_DEFINITION}.
   */
  ...
  /**
   * Returns the full stacktrace of the exception that occurs when the historic job log with the given id was last executed. Returns null
  ...
  /**
   * Returns the full error details that occurs when the historic external task log with the given id was last executed. Returns null when
   * the historic external task log contains no error details.
   *
   ...
   * {@link SetRemovalTimeSelectModeForHistoricProcessInstancesBuilder#absoluteRemovalTime(Date)}. Pass {@code null} to clear the removal
   * time.
   *
   * <p>As an alternative, the removal time can also be calculated via
   * {@link SetRemovalTimeSelectModeForHistoricProcessInstancesBuilder#calculatedRemovalTime()} based on the configured time to live
   * values.
   *
   * <p>To additionally take those historic process instances into account that are part of
   * a hierarchy, enable the flag {@link SetRemovalTimeToHistoricProcessInstancesBuilder#hierarchical()}
   *
   ...
   *
   * <p>An absolute time can be specified via
   * {@link SetRemovalTimeSelectModeForHistoricDecisionInstancesBuilder#absoluteRemovalTime(Date)}. Pass {@code null} to clear the removal
   * time.
   *
   * <p>As an alternative, the removal time can also be calculated via
   * {@link SetRemovalTimeSelectModeForHistoricDecisionInstancesBuilder#calculatedRemovalTime()} based on the configured time to live
   * values.
   *
   ...
   *
   * <p>An absolute time can be specified via
   * {@link SetRemovalTimeSelectModeForHistoricBatchesBuilder#absoluteRemovalTime(Date)}. Pass {@code null} to clear the removal time.
   *
   * <p>As an alternative, the removal time can also be calculated via
   * {@link SetRemovalTimeSelectModeForHistoricBatchesBuilder#calculatedRemovalTime()} based on the configured time to live values.
   *
   * <p>To create the batch and complete the configuration chain, call
   ...
   * @throws AuthorizationException <ul>
   * <li>
   * when no {@link ProcessDefinitionPermissions#UPDATE_HISTORY} permission is granted on {@link Resources#PROCESS_DEFINITION}</li>
   * <li>
